...
Wir haben bereits in Abschnitt \ref{subsubsec:parser} von Tokens geredet. Von Tokens spricht man insbesondere bei einer Sprache die durch Whitespaces getrennt werden. Ein Token ist hierbei die kleinste Sequenz von Zeichen die für die Grammatik eine Bedeutung hat.
...
Q? & \text{entweder $A=Q$ oder $A=\varepsilon$} \\
Q* & \text{beliebige Konkatenation von $Q$ mit sich selbst, d.h. $A \in \gbr{\varepsilon,Q,QQ,\ldots}$} \\
Q+ & \text{Gleiche Bedeutung wie $QQ*$, d.h. mindestens ein $Q$} \\
(Q) & \text{Klammerung des Ausdrucks $Q$} \\
\end{cases}\]
\end{defn}

\subsubsection{Grammatikalische Analyse}
