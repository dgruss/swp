...
Wir haben eine Eingabe (mglw. Wort der Sprache) und beginnen im Zustand $S$. Die einzigen gültigen Übergänge sind die zu Zustand $A$. Es muss also entweder eine $0$ oder
...
\newpage
\begin{bsp} \qquad \\
\begin{tabular}{p{0.75\hsize}p{0.2\hsize}}
...
\ifthenelse{\boolean{long}}{\end{enumerate}}{\end{compactenum}}
\end{tabular}\\\begin{tabular}{p{0.75\hsize}p{0.2\hsize}}
...
\begin{bsp} \qquad \\
\begin{tabular}{p{0.7\hsize}p{0.25\hsize}}
...
In der Grammatik sind keine direkten Linksrekursionen. Regeln 2, 5 und 6 haben einen gemeinsamen Prefix ($\u{(}$).
Man sieht auch recht schnell dass durch die Produktionen $E \to G \to E \u{*} E$ eine indirekte Linksrekursion entsteht.
\end{tabular}\\
\vfill
\begin{tabular}{p{0.7\hsize}p{0.25\hsize}}
Wenn wir uns überlegen wie man die gemeinsamen Prefixe entfernen kann wird man intuitiv auf die gleiche Idee kommen
...
\vspace{-0.75cm}\ifthenelse{\boolean{long}}{\begin{enumerate}}{\begin{compactenum}}
\item $E \to G$
\item $G \to E \u{+} E$
\item $G \to E \u{*} E$
...
\ifthenelse{\boolean{long}}{\end{enumerate}}{\end{compactenum}}
\end{tabular}\\\begin{tabular}{p{0.65\hsize}p{0.3\hsize}}
...
Lösen wir zunächst wieder die gemeinsamen Prefixe (Regeln 6-9).
Dadurch entstehen wieder neue gemeinsame Prefixe die dann gelöst werden müssen (Regeln 7-10): \\
...
\end{array}\]
Wir beginnen mit der leeren Parse-Tabelle. Wir haben eine Zeile pro Non-Terminal und
...
\hline
  & \u{a} & \u{b} & \u{c} & \u{q} & \$ \\\hline
A & \color{red}{A \to A \u{a}} & A \to \u{b}   &       & \color{red}{A \to A \u{a}} &    \\
  &               & \color{red}{A \to A \u{a}} &       &               &    \\\hline
B &               &               &       &               &    \\\hline
C &               &               &       &               &    \\ \hline
\end{array} \to \begin{array}{|l|l|l|l|l|l|}
\hline
  & \u{a} & \u{b} & \u{c} & \u{q} & \$ \\\hline
A & A \to A \u{a} & A \to \u{b}   &       & A \to A \u{a} &    \\
  & \color{red}{A \to A B C}   & A \to A \u{a} &       & \color{red}{A \to A B C}   &    \\
  &               & \color{red}{A \to A B C}   &       &               &    \\\hline
B &               &               &       &               &    \\\hline
C &               &               &       &               &    \\ \hline
\end{array}\]
...
\begin{algo}[LL(1)-Parsing mit Tabelle]
...
\end{array}\]
Diesmal haben wir keine Mehrfach-Einträge, die Grammatik ist also eine LL(1)-Grammatik. Die Parsing-Tabelle für $\u{abbqa}$ sieht dann wie folgt aus:
\[\begin{array}{|l|r|l|}
\hline
...
\$ A A \u{a}          & \u{abbqa}\$     &  \\
\$ A A                &  \u{bbqa}\$     & A \to BCA \\
\$ A A A C B          &  \u{bbqa}\$     & B \to \u{b} \\
\$ A A A C \u{b}      & \u{bbqa}\$     &  \\
\$ A A A C            & \u{bqa}\$     & C \to A \u{c} \\
\$ A A A \u{c} A      & \u{bqa}\$     & A \to BCA \\
\$ A A A \u{c} ACB    & \u{bqa}\$     & B \to \u{b} \\
\$ A A A \u{c} AC\u{b}& \u{bqa}\$     &  \\
\$ A A A \u{c} AC     & \u{qa}\$     & C \to A \u{c} \\
\$ A A A \u{c} A\u{c}A     & \u{qa}\$     & A \to BCA \\
\$ A A A \u{c} A\u{c}ACB     & \u{qa}\$     & B \to \u{q} \\
\$ A A A \u{c} A\u{c}AC\u{q}     & \u{qa}\$     &  \\
\$ A A A \u{c} A\u{c}AC     & \u{a}\$     & C \to A \u{c} \\
\$ A A A \u{c} A\u{c}A\u{c}A     & \u{a}\$     & A \to \u{a}A \\
\$ A A A \u{c} A\u{c}A\u{c}A\u{a}     & \u{a}\$     &  \\
\$ A A A \u{c} A\u{c}A\u{c}A     & \$     & A \to \varepsilon \\
\$ A A A \u{c} A\u{c}A\u{c}     & \$     & \text{FAIL} \\ \hline
\end{array}\]
Wir haben somit \textbf{widerlegt} dass das Wort $\u{abc}$ von der gegebenen Grammatik akzeptiert wird.
\[\begin{array}{|l|r|l|}
\hline
\textbf{Stack}        & \textbf{Input} & \textbf{Produktion} \\ \hline
\$ A                  & \u{abc}\$     & A \to \u{a} A \\
\$ A A \u{a}          & \u{abc}\$     &  \\
\$ A A                &  \u{bc}\$     & A \to BCA \\
\$ A A A C B          &  \u{bc}\$     & B \to \u{b} \\
\$ A A A C \u{b}      & \u{bc}\$     &  \\
\$ A A A C            & \u{c}\$     & C \to A \u{c} \\
\$ A A A \u{c} A      & \u{c}\$     & A \to \varepsilon \\
\$ A A A \u{c}        & \u{c}\$     & \\
...
\$                    & \$          & \text{ACCEPT} \\ \hline
\end{array}\]
Und mit dieser Parsing-Tabelle haben wir \textbf{gezeigt} dass das Wort $\u{abc}$ von der gegebenen Grammatik akzeptiert wird.
\end{bsp}
\ifthenelse{\boolean{long}}{}{\end{comment}}
