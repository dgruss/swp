...
\begin{align*}
\delta\tu{reverse(x)}&=\tu{if atom?(x) then x reverse2(x,nil)} \\
...
Die Korrektheit von $\delta\u{\text{max}}$ wurde per Induktion gezeigt.

$\delta\u{\text{depth}}$ wurde für $\text{atom?}(\text{first}(\omega(\u{x})))=T$ gezeigt unter der Voraussetzung der Korrektheit für $\delta\u{\text{depth}}$ mit $\text{atom?}(\text{first}(\omega(\u{x})))=F$.
...
Wir definieren:
\[\abs{\omega(\u{x})} = \begin{cases}1 & eq?(\omega(\u{x}),nil) \\
...
\intertext{\textcolor{darkblue}{NR: $I(\delta,\omega,\u{\text{atom?(first(x))}})=\text{atom?}(I(\delta,\omega,\u{\text{first(x)}}))=\text{atom?(first}(I(\delta,\omega,\u{\text{x}})))=\text{atom?(first(nil))}=\text{atom?(nil)}=F$ (siehe Fallunterscheidung)}}
&&=I(\delta,\omega,&\u{\text{if eq?(x,nil) then nil}} \\ &&&\u{\text{else max(build(depth(first(x)),nil),depth(rest(x)))}})
\intertext{\textcolor{darkblue}{Da $\abs{\omega(\u{x})}=1$ ist muss $\omega(\u{x})=\text{nil}$ sein.}}
&&=I(\delta,\omega,&\u{\text{nil}})=\text{nil}=f_{d}(\text{nil})=f_{d}(\omega(\u{x}))
\end{align*}
...
&&=&\text{max}(\text{build}(I(\delta,\omega,\u{\text{depth(first(x))}}),\text{nil}),I(\delta,\omega,\u{\text{depth(rest(x))}}))
...
&&=&\text{max}(\text{build}(I(\delta,\omega',\u{\text{depth(x)}}),\text{nil}),I(\delta,\omega'',\u{\text{depth(x)}}))
...
&&=&\text{max}(\text{build}(f_{d}(\omega'(\u{x})),\text{nil}),f_{d}(\omega''(\u{x})))
\end{align*}

Die maximale Verschachtelungstiefe kann nur in $\text{first}(\u{x})$ oder $\text{rest}(\u{x})$ erreicht werden.
Daher machen wir eine Fallunterscheidung die diese beiden Fälle unterscheidet (und damit alle Fälle abdeckt):
\begin{itemize}
\item Wird die maximale Verschachtelungstiefe $k$ (Anmerkung: $f_{d}(\omega(\u{x}))=\pi(k)$) in $\omega(\u{x})$ in $\text{first}(\u{x})$ erreicht
so ist $f_{d}(\omega'(\u{x}))=\pi(k-1)$, da $\omega'(\u{x})$ eine Verschachtelungsebene weniger enthält (die der Eltern-Liste).
Dann ist $\text{build}(f_{d}(\omega'(\u{x})),\text{nil})=\pi(k-1+1)=\pi(k)$.

In diesem Fall gilt also $\text{max}(\text{build}(f_{d}(\omega'(\u{x})),\text{nil}),f_{d}(\omega''(\u{x}))) = f_{d}(\omega(\u{x})) = \pi(k)$.
\item Andernfalls wird die maximale Verschachtelungstiefe $k$ in $\omega(\u{x})$ in $\text{rest}(\u{x})$ erreicht. \\
...
In diesem Fall gilt also ebenfalls $\text{max}(\text{build}(f_{d}(\omega'(\u{x})),\text{nil}),f_{d}(\omega''(\u{x}))) = f_{d}(\omega(\u{x})) = \pi(k)$.
\end{itemize}
Daraus folgt
\[\text{max}(\text{build}(f_{d}(\omega'(\u{x})),\text{nil}),f_{d}(\omega''(\u{x})))=f_{d}(\omega(\u{x})).\]
...
\[\tforall \omega(\tu{y}) \leq n \leq \abs{\omega(\tu{x})}: \quad \I{E}{\delta,\omega}{nth(x,y)}=\ell_{\omega(\tu{y})}
...
\intertext{\textcolor{darkblue}{NR: $\I{E}{\delta,\omega}{eq?(y)}=\text{eq?}\rbr{\I{E}{\delta,\omega}{y},\I{E}{\delta,\omega}{eins}}=\text{eq?}\rbr{\omega(\tu{y}),1}=\text{eq?}\rbr{1,1}=T$}}
...
&\quad \I{E}{\delta,\omega}{nth(x,y)} \\
&=\I{E}{\delta,\omega}{if eq?(y,eins) then first(x) else nth(rest(x),minus(y,eins))}
\intertext{\textcolor{darkblue}{NR: $\I{E}{\delta,\omega}{eq?(y)}=\text{eq?}\rbr{\I{E}{\delta,\omega}{y},\I{E}{\delta,\omega}{eins}}=\text{eq?}\rbr{\omega(\tu{y}),1}=\text{eq?}\rbr{n+1,1}=F$}}
&=\I{E}{\delta,\omega}{nth(rest(x),minus(y,eins))}
\intertext{\textcolor{darkblue}{NE: $\omega'(\tu{x})=\I{E}{\delta,\omega}{rest(x)}=\text{rest}\rbr{\I{E}{\delta,\omega}{x}}=\text{rest}\rbr{\omega(\tu{x})}=\text{rest}\rbr{\ebr{\ell_1 \ell_2 \ldots}}=\ebr{\ell_2 \ldots}=\ebr{\ell'_1 \ldots}$\newline
                                    Wir definieren hier $\tforall i: \ell'_{i} = \ell_{i+1}$.
                                    \newline
                                    $\omega'(\tu{y})=\I{E}{\delta,\omega}{minus(y,eins)}=\text{minus}\rbr{\I{E}{\delta,\omega}{y},\I{E}{\delta,\omega}{eins}}=\text{minus}\rbr{\omega(\tu{y}),1}=\text{minus}\rbr{n+1,1}=n$}}
...
\begin{\whichitem}
\item $\tforall x \in A, f_i \in F: \pi\ebr{f_i}\rbr{\pi\rbr{x}} = \pi\rbr{f_i\rbr{x}}$, d.h.
für alle Werte $x$ in $A$ und alle Funktionen in $F$ darf es keinen Einfluss auf das Ergebnis haben,
ob der Wert $x$ zuerst kodiert und dann die kodierte Funktion mit diesem berechnet wird
oder zuerst der die Funktion mit dem Wert $x$ berechnet und das Ergebnis kodiert wird.
\item $\tforall x \in A, p_i \in P: \pi\ebr{p_i}\rbr{\pi\rbr{x}} \lra \pi\rbr{p_i\rbr{x}}$, gleich wie bei den Funktionen
\end{\whichitem}
\end{defn}
...
\begin{defn}[Dekodierungs-Funktion $\rho$]
$\rho$ ist die Umkehrfunktion zur Mapping-Funktion $\pi$.
Es gilt $\rho(\pi(x))=x$.
\end{defn}
...
\[\I{E}{\delta,\omega}{IX(delta,omega,exp)}=\pi_{A}\rbr{\I{E}{\rho_{\text{FENV}}(\omega(\tu{delta})),\rho_{ENV}(\omega(\tu{delta}))}{$\rho_{\lng{E}}(\omega(\tu{exp}))$}}\]
...
Diese Definition basiert auf der Definition der Sprache der Terme $\lng{T}$.
...
\textit{Lösung:}
\begin{align*}
&\quad \I{P}{\omega}{((lt?(x,plus(eins,eins)) \twedge eq?(x,null)) \tvee lt?(x,eins)) \tto eq?(x,eins)} \\
&\lra \I{P}{\omega}{((lt?(x,plus(eins,eins)) \twedge eq?(x,null)) \tvee lt?(x,eins))} \tto \I{P}{\omega}{eq?(x,eins)} \\
&\lra \rbr{\I{P}{\omega}{(lt?(x,plus(eins,eins)) \twedge eq?(x,null)) \tvee lt?(x,eins)}} \tto eq?(\I{T}{\omega}{x},\I{T}{\omega}{eins}) \\
&\lra \rbr{\I{P}{\omega}{(lt?(x,plus(eins,eins)) \twedge eq?(x,null)) \tvee lt?(x,eins)}} \tto eq?(\omega(\u{x}),1) \\
&\lra \rbr{\I{P}{\omega}{(lt?(x,plus(eins,eins)) \twedge eq?(x,null)) \tvee lt?(x,eins)}} \tto eq?(0,1) \\
&\lra \rbr{\I{P}{\omega}{(lt?(x,plus(eins,eins)) \twedge eq?(x,null)) \tvee lt?(x,eins)}} \tto F \\
...
In Tabelle~\ref{unterschiede_funktional_imperativ}
\begin{table}[h!t]
 \centering
...
=\tneg F
=T$}}
&= \Iu{A}{\I{A}{\omega^{1}}{begin z := plus(z,x) ; y := minus(y,eins) end}}{\tu{while \tneg eq?(y,null) do} \; Z_{4,5,6,7}} \\
&= \Iu{A}{\I{A}{\I{A}{\omega^{1}}{z := plus(z,x)}}{y := minus(y,eins)}}{\tu{while \tneg eq?(y,null) do} \; Z_{4,5,6,7}} \\
...
=\tneg F
=T$}}
&= \Iu{A}{\I{A}{\omega^{3}}{begin z := plus(z,x) ; y := minus(y,eins) end}}{\tu{while \tneg eq?(y,null) do} \; Z_{4,5,6,7}} \\
&= \Iu{A}{\I{A}{\I{A}{\omega^{3}}{z := plus(z,x)}}{y := minus(y,eins)}}{\tu{while \tneg eq?(y,null) do} \; Z_{4,5,6,7}} \\
...
haben wir nun Schleifen. Über diese können wir auch vollständige Induktionen durchführen.
In $\lng{A}$ ist dies aufgrund des geringen Umfangs wesentlich einfacher als
...
Eine einfache aber mächtige Erweiterung ist das Konzept ``Pointer'' (``Zeiger'').
Variablen werden nun nur noch Adressen enthalten, die Werte stehen in einem neuen
Environment an der entsprechenden Stelle.
...
\item Schleifen:
\begin{\whichenum}
\item $\Iu{A^P}{\omega}{\tu{while} \; p \; \tu{do} \; a} = \Iu{A^P}{\Iu{A^P}{\omega,\sigma}{a}}{\tu{while} \; p \; \tu{do} \; a}$ wenn $\Iu{P^P}{\omega,\sigma}{p}=T$.
\item $\Iu{A^P}{\omega}{\tu{while} \; p \; \tu{do} \; a} = \omega$ wenn $\Iu{P^P}{\omega,\sigma}{p}=F$.
\end{\whichenum}
\item Pointer Assignments:
$\Iu{A^P}{\omega}{\tu{ref} \; v \; \tu{:= ref} \; w} = (\omega',\sigma)$ mit $\omega \sim_v \omega'$ und $\omega'(v)=\omega(w)$.
\end{\whichenum}
\end{defn}
